    ...
    .then(
      (result) => {
        if (status === 201 || status === 200) {
          if (history) {
            if (result.token.includes("admin")) {
...
export const reqOptions = (method, data, bearer = null) => {
  let myHeaders = {
    "Instincthub-sk-header": "22-072021kidbackendyste3333ifkIks304",
    Origin: window.location.origin,
    "Content-Type": "application/json",
  };
  console.log("myHeaders", myHeaders);
  if (bearer) {
    let access = JSON.parse(getCookie("access")).access;
...
export const reqOptionFor = (method, data, bearer = null) => {
  let myHeaders = new Headers();
  myHeaders.append("Instincthub-sk-header", SK_VALUE());
...
export const HOST_URL = () => {
  return "https://sandboxapi.instincthub.com";
};
  ...
  const eachInput = (key, value, index) => {
    ...
    if (error_tag) {
      ...
      if (key === "user") {
        ...
        if (
          inputField.getAttribute("type") === "radio" ||
          inputField.classList.contains("dropdown")
        ) {
          // console.log(key)
          inputField.parentElement.parentElement.parentElement.append(span_tag);
        } else if (inputField.getAttribute("type") === "checkbox") {
          inputField.parentElement.parentElement.parentElement.parentElement.append(
            span_tag
          );
        } else if (inputField.getAttribute("type") === "file") {
          // console.log(key)
